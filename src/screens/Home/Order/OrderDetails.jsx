    ...
    return (
        <SafeAreaView className='flex-1 bg-white'>
            ...
            <View className="flex-1">
                {loading ? (
                    <View className="flex-1 justify-center items-center mb-[50]">
                        <LottieView
                    ...
                    <ScrollView className="flex-1">
                        <View className='px-4 pb-10'>
                                        ...
                                        {orderDetails?.data?.data?.plan_info.map((elm, ind) => (
                                            <View key={ind} className='mb-[2]'>
                                                ...
                                                <Text className="text-[16px] poppins-medium txt-grey-600">
                                                    {elm.veg_count > 0 && elm.veg_count + ' Plate'} (Veg)
                                                    {elm.nveg_count > 0 && ' and ' + elm.nveg_count + ' (Non-veg)'}
                                                </Text>
                                            </View>
                                        ))}
                                    </View>
                            ...
                            <TouchableOpacity
                                onPress={() => navigation.navigate('OrderManagement', { orderId: orderData?.kitchen_order_id })}
